...
########### 循环
setwd('/home/wus/2023-2-23_008_TCGAbiolinks_download/box-FBXO22/5_23')
...
p <-  ggboxplot(data4, x="project_id", y="ENSG00000167196.14", color = "sample_type",bxp.errorbar =TRUE,outlier.shape = NA)#,order=)
...
stat.test
 ...
 bxp <- ggboxplot(
    data4, x="project_id", y="ENSG00000167196.14", color = "sample_type",
    palette=c("blue","pink"))
...
#data4<-data3[order(data3$fc, decreasing = TRUE), ]
 bxp <- ggboxplot(
    data4, x="project_id", y="ENSG00000167196.14", color = "sample_type",bxp.errorbar =TRUE,outlier.shape = NA)
#bxp
# add p value 并且确定xy的位置
stat.test <- stat.test %>% 
