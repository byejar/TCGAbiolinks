...
########### 循环
setwd('/home/wus/2023-2-23_008_TCGAbiolinks_download/box-FBXO22/')
csv_file='merge-v2.txt'
...
#### 基因名转换
gene_id<-rowData(data)[rowData(data)$gene_name=="FBXO22",'gene_id']
print(gene_id)
...
NT_mean=mean(df3[df3$sample_type == 'Primary Tumor',gene_id])
TP_mean=mean(df3[df3$sample_type == 'Solid Tissue Normal',gene_id])
...
library(ggplot2)
data <- read.table(csv_file,header = T)

data2 <- subset(data, patient != 'patient')

data2 <- subset(data2, project_id != 'TCGA-SKCM')

write.csv(data2,'data2-v2.csv')

#################
data3 <- read.csv("data2-v2.csv",header = T)
...
# p <-  ggboxplot(data4, x="project_id", y=gene_id, color = "sample_type",bxp.errorbar =TRUE,outlier.shape = NA)#,order=)
...
# stat.test <- data4 %>% 
#   group_by(fc) %>%
#   t_test(ENSG00000077254.14 ~ sample_type) %>%
...
#  bxp <- ggboxplot(
#     data4, x="project_id", y=gene_id, color = "sample_type",
#     palette=c("blue","pink"))
...
# #data4<-data3[order(data3$fc, decreasing = TRUE), ]
#  bxp <- ggboxplot(
#     data4, x="project_id", y=gene_id, color = "sample_type",bxp.errorbar =TRUE,outlier.shape = NA)
# #bxp
# # add p value 并且确定xy的位置
# stat.test <- stat.test %>% 
...
#   ) + 
# scale_y_continuous(expand = expansion(mult = c(0, .5)))+ 
#  theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1))+ylab("USP33_TPM")

#   ggsave(fig_file_txt,dpi=300,width=15,height=7)



# fig_file='box_no_txt.png'
# bxp+ 
#  theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1))+ylab("USP33_TPM")  + ylim(0,200)
# ggsave(fig_file,dpi=300,width=15,height=7)

# ####改颜色
# fig_file_txt_c='box_txt-color.png'
...
# fig_file_c='box_no_txt-color.png'
# bxp+
...
stat.test <- data4 %>% 
  group_by(fc) %>%
  t_test(ENSG00000167196.14 ~ sample_type) %>%
  adjust_pvalue(method = "bonferroni") %>% 
  add_significance("p.adj")
...
####改颜色
fig_file_txt_c='box_txt-color-NT.png'
...
scale_y_continuous(expand = expansion(mult = c(0, .5)))+ 
 theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1))+ylab("FBXO22_TPM")

  ggsave(fig_file_txt_c,dpi=300,width=15,height=7)



fig_file_c='box_no_txt-color-NT.png'
bxp+
    scale_color_manual(values = c( "#0b5d0a","#9a0000"))+ 
 theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1))+ylab("FBXO22_TPM")  + ylim(0,200)
ggsave(fig_file_c,dpi=300,width=15,height=7)


####### 改变长宽

fig_file_c9='box_no_txt-color-NT-9.png'
bxp+
    scale_color_manual(values = c( "#0b5d0a","#9a0000"))+ 
 theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1))+ylab("FBXO22_TPM")  + ylim(0,30)
...
s
a=s[c('project_id','fc','n1','n2','p')]
write.csv(a,'stat_test.csv')

#write.csv(d4,'data4.csv')
