...
################################### 循环  去重复 加p值显示 正式使用 （颜色 注释 线粗细都合适） 保存两个版本的图片 
projects <- getGDCprojects()
...
###
fig_file=paste('/home/wus/2023-2-23_008_TCGAbiolinks_download/pairline/',projects$project_id[i],"_paired_points-NT-dropdu-p.png")
###
fig_file_txt=paste('/home/wus/2023-2-23_008_TCGAbiolinks_download/pairline/',projects$project_id[i],"_paired_points_txt-NT-dropdu.png")
####### 配对数据构建
TCGA_rda<-paste0("/home/wus/2023-2-23_008_TCGAbiolinks_download/TCGA_RNA_data/",projects$project_id[i],".rda")
...
#去重复
df3<-df3[!duplicated(rleidv(df3, cols = c("sample_type", "patient"))), ]
...
ggplot(df3,aes(sample_type,ENSG00000168883.20))+
  ...
  #y轴用自己写的标注
  #图注用自己写的
  labs(title = paste0(projects$project_id[i],'  (N=T=',length(df3[,1])/2,')'),subtitle = paste0('p=',stat.test$p))
    #write.csv(dfu,csv_file)
...
ggplot(df3,aes(sample_type,ENSG00000168883.20))+
  ...
  #y轴用自己写的标注
  #图注用自己写的
  labs(title = paste0(projects$project_id[i],'  (N=T=',length(df3[,1])/2,')'))
...
as.data.frame() %>% 
select(project_id,tumor) %>% 
  filter(grepl(pattern="BLCA",project_id))
projects
...
###
fig_file=paste('/home/wus/2023-2-23_008_TCGAbiolinks_download/pairline/',projects$project_id[i],"_paired_points-v2.png")
###
fig_file_txt=paste('/home/wus/2023-2-23_008_TCGAbiolinks_download/pairline/',projects$project_id[i],"_paired_points_txt-v2.png")
####### 配对数据构建
TCGA_rda<-paste0("/home/wus/2023-2-23_008_TCGAbiolinks_download/TCGA_RNA_data/",projects$project_id[i],".rda")
...
df3<-df3[!df3$Row.names %in% c('TCGA-BL-A13J-01A-11R-A10U-07') , ]
print(df3)
#### 添加NT顺序
df3$sample_type <-as.factor(df3$sample_type)
df3$sample_type <- factor(df3$sample_type, levels=rev(levels(df3$sample_type)))
